...
async function loadAnimeShowsForSection(section) {
  ...
  let page = Math.floor(animeLoadedCounts[section.slug] / animeShowsPerLoad) + 1;
  let appended = 0;
  let hasMore = true;

  while (appended < animeShowsPerLoad && hasMore) {
    ...
    for (const animeShow of uniqueShows) {
      if (appended >= animeShowsPerLoad) break;
