document.addEventListener('DOMContentLoaded', () => {
  ...
  function getVisibleItems() {
    ...
    if (state.sortOrder === 'old') items.reverse();

    return items;
  }

  function syncActionState() {
    clearAllBtn.textContent = state.activeType === 'search' ? 'Clear all search history' : 'Clear all watch history';
  ...
  function renderHistoryItem(item) {
    ...
    deleteBtn.addEventListener('click', () => {
      ...
      const next = loadList(getActiveKey()).filter(entry => getItemId(entry) !== itemId);
